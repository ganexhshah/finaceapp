...
export default function ExpenseDetailsModal({ visible, onClose, expense, onUpdated, onDeleted }: ExpenseDetailsModalProps) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.success) {
        setAlertConfig({ title: 'Failed', message: res.message || 'Failed to update expense', type: 'error' });
        setShowAlert(true);
        return;
      }

      setAlertConfig({ title: 'Success', message: 'Expense updated successfully!', type: 'success' });
      setShowAlert(true);
      setIsEditing(false);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      const res = await api.deleteExpense(expense.id);

      if (!res.success) {
        setAlertConfig({ title: 'Failed', message: res.message || 'Failed to delete expense', type: 'error' });
        setShowAlert(true);
        setDeleting(false);
        return;
      }

      setAlertConfig({ title: 'Success', message: 'Expense deleted successfully!', type: 'success' });
      setShowAlert(true);
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.categorySection}>
              <View style={styles.categoryIcon}>
                <Ionicons 
                  name={(expense.category?.icon || 'cart') as any} 
                  size={32} 
                  color="#ef4444" 
                />
              </View>
              <Text style={styles.categoryName}>{expense.category?.name || 'Expense'}</Text>
              {expense.recurring && (
                <View style={styles.recurringBadge}>
                  <Ionicons name="repeat" size={14} color="#ef4444" />
            ...
            <View style={styles.inputGroup}>
              ...
              {isEditing ? (
                ...
                <Text style={styles.valueText}>{expense.title}</Text>
              )}
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <Text style={styles.valueText}>{expense.date}</Text>
            </View>
            ...
            {expense.account && (
              <View style={styles.inputGroup}>
                ...
                <Text style={styles.valueText}>{expense.account.name}</Text>
              </View>
            )}
            ...
            <View style={styles.inputGroup}>
              ...
              {isEditing ? (
                ...
                <Text style={styles.valueText}>{expense.notes || 'No notes'}</Text>
              )}
            </View>
          </ScrollView>
