...
export default function AddIncomeModal({ visible, onClose, onSaved }: AddIncomeModalProps) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      setLoadingMeta(true);
      try {
        const [catRes, accRes] = await Promise.all([api.getCategories('income'), api.getAccounts()]);
        if (!mounted) return;
        ...
        if (accRes.success && accRes.data) {
          ...
          // Keep account optional; don't auto-select to avoid surprising writes.
        }
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.success) {
        setAlertConfig({ title: 'Failed', message: response.message || 'Failed to create income', type: 'error' });
        setShowAlert(true);
        return;
      }
      ...
      setAlertConfig({ title: 'Error', message: 'Something went wrong while saving income', type: 'error' });
      setShowAlert(true);
    } finally {
      setSaving(false);
    }
          ...
          <ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
                ...
                {loadingMeta && categories.length === 0 ? (
                  ...
                  >
                    <Ionicons
                      ...
                      size={24}
                      color={selectedCategoryId === category.id ? '#10b981' : '#6b7280'}
                    />
                    <Text
                      style={[
                        styles.categoryText,
            ...
            >
              <View style={styles.recurringLeft}>
                ...
                <Text style={styles.recurringLabel}>Recurring Income</Text>
              </View>
